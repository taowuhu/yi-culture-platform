...
export function calculateHexagram(lines) {
    ...
    // 2. 转换本卦（保持原始阴阳状态）
    const benGuaLines = lines.map(lineToYinYang);
    // 3. 检查动爻并构建变卦（如果有动爻）
    const movingLines = []; // 存储 1~6 的动爻位置（对外接口用）
    let bianGuaLines;
    for (let i = 0; i < 6; i++) {
        const line = lines[i];
...
export function identifyTrigram(lines) {
    // 1. 验证输入长度
    if (lines.length !== 3) {
...
export function identifyHexagramTrigrams(lines) {
    // 1. 验证输入长度
    if (lines.length !== 6) {
        throw new Error('输入必须恰好包含6个爻，当前长度: ' + lines.length);
    }
